function miner_depth_k_means(X)
% figure
...
N = length(X); % 点的个数
Y=ones(1,N);%为了便于表示一维数据聚类情况，将y坐标统一成1，便于在二维图上的理解
...
%%
figure;%每次迭代的结果绘图，若只最终迭代结果，则把这部分代码放到最后，figure(m)改成figure
...
xlabel('X');
ylabel('Y');
title(['聚类',num2str(m)-1,'次之后的数据点'],'FontSize',15);
...
% hold on
% plot(C(1,indexC) , C(2,indexC), 'ok'); %作出属于第三类点的图形
% hold on
%  
