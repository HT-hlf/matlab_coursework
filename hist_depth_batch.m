...
% pt = 'Z:\doing\Matlab\matlab_coursework\recordData_process_annotation_sum\depth_val';
rootpath =  'Z:\doing\Matlab\matlab_coursework\recordData_process_annotation_sum\';% 图像文件夹路径  
depthpath='depth_val/';
labelpath='labels/';
subdir=dir('Z:\doing\Matlab\matlab_coursework\recordData_process_annotation_sum\depth_val');
% for i = 1:length(subdir)
for i = 1:50
    if( isequal( subdir( i ).name, '.' ) ||  isequal( subdir( i ).name, '..' )  )   % 如果不是目录 就跳过
        continue;
    end
    saveimage_path=['Z:\doing\Matlab\matlab_coursework\save_images\',int2str(i),'.jpg'];
    ...
    subplot(2, 3, 5); imshow(dest, []);f=getframe(gcf);
    imwrite(dest,saveimage_path); title('矿工检测框');
