...
[resX,resY,record] = ht_mean(I_bbox_thr_one_d,I_bbox_thr_one_d,3);% record 中存放着每一个类别组的成员数量 
...
for i = 1:length(record)
    plot(resX(i,1:record(i)),resY(i,1:record(i)),ht_color(i))
end
% 下面是标记出每一个类别的类别代表点
for i = 1:length(record)
