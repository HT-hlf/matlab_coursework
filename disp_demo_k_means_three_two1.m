...
%         data(:,data_i)=[ht_j,ht_k,I_bbox_thr(ht_j,ht_k)];
        data(:,data_i)=[power(power(ht_j-h_bbox/2,2)+power(ht_k-w_bbox/2,2),0.5),I_bbox_thr(ht_j,ht_k)];
        data_i=data_i+1;
    end
end
...
if mean(res(record_max_ind_c*2,1:record(record_max_ind_c)),2)<5
    depth_i=3-record_max_ind_c
else
    depth_i=record_max_ind_c
end
depth=round(mean(res(depth_i*2,1:record(depth_i)),2)/255*780);
ht_color=['*';'+';'o']
if h/k == 2
    hold on
    for i = 1:k
        plot(res(i*2-1,1:record(i)),res(i*2,1:record(i)),ht_color(i))
        plot(mean(res(i*2-1,1:record(i)),2),mean(res(i*2,1:record(i)),2),'Marker','square','Color','k','MarkerFaceColor','k','LineStyle','none')
    end
    hold off
elseif h/k == 3
    for i = 1:k
        plot3(res(i*3-2,1:record(i)),res(i*3-1,1:record(i)),res(i*3,1:record(i)),ht_color(i))
