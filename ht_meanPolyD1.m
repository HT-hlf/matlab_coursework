function [ res, record] = ht_meanPolyD1(data,k,h_bbox,w_bbox)
% 功能：
%     实现多维空间k-mean聚类算法
% 输入：
...
%     seed_ht = zeros(cntOfDimension,k);
%     oldSeed_ht = zeros(cntOfDimension,k);
...
%                     b = b + power(data(row,i)-seed(row,j),2);
                  ...
                  b=power(power(power(data(1,i)-h_bbox/2,2)+power(data(2,i)-w_bbox/2,2),0.5)-seed(1,j),2)+power(data(3,i)-seed(3,j),2);
